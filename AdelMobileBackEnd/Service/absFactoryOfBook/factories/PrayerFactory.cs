        ...
        public  async override Task<absBook> GetBook()
        {
            try
            {
                using (HttpClientHandler handler = new HttpClientHandler { AllowAutoRedirect = false, AutomaticDecompression = DecompressionMethods.All })
                {
                    ...
                    using (HttpClient client = new HttpClient(handler))
                    {
                        ...
                        using (HttpResponseMessage response = await client.GetAsync("https://ficbook.net/readfic/10063965"))
                        {
                            ...
                            return new Prayer(titleHtml, int.Parse(commentsHtml), int.Parse(likeHtml));
                        }
                    }
                }
            }
            catch (AggregateException exs)
            {
                foreach (var e in exs.InnerExceptions)
                    await Log.LoggingAsync(e, "GetPrayerAsync");
                return null;
            }
        }
    }
