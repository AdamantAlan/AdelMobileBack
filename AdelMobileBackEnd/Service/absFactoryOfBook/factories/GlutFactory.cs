...
{
    public class GlutFactory : FactoryOfBook
    {
            ...
            try
            {
                using (HttpClientHandler handler = new HttpClientHandler { AllowAutoRedirect = false, AutomaticDecompression = DecompressionMethods.All })
                {
                    ...
                    using (HttpClient client = new HttpClient(handler))
                    {
                        ...
                        using (HttpResponseMessage response = await client.GetAsync("https://ficbook.net/readfic/10628564"))
                        {
                            ...
                            return new Glut(titleHtml, int.Parse(commentsHtml), int.Parse(likeHtml));
                        }
                    }
                }
            }
